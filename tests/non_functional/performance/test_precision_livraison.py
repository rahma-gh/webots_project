"""Tests non fonctionnels (performance) : précision de livraison."""
...
def test_precision_livraison():
    ...
    assert r.get("final_distance",999) < 0.05, \
        f"Précision insuffisante : {r.get('final_distance'):.4f}m"
    print(f" Précision : {r.get('final_distance'):.4f}m < 0.05m")
